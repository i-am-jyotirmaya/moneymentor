...
export function JudgementReportsPanel({
  ...
  const cadence: JudgementReportCadence =
    params.get("cadence") === "Monthly" ? "Monthly" : "Weekly";
  const scope: JudgementReportScope =
    params.get("scope") === "Household" ? "Household" : "Personal";
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      const request = { householdId, scope: effectiveScope, cadence, period };
      void Promise.all([
        getJudgementReport(accessToken, request).catch(
          (caughtError: unknown) => {
        ...
        .then(([latest, reportHistory, findings]) => {
          if (!active) return;
          const allReports =
            latest && !reportHistory.some((item) => item.id === latest.id)
  ...
  }, [accessToken, cadence, effectiveScope, householdId, period]);
  ...
  const categoryChanges = useMemo(
    () =>
      [...(report?.categories ?? [])]
        .filter(
          (category) =>
            category.isMaterial || category.baselineTrend === "NewActivity",
        )
        .sort(
          (left, right) =>
            Math.abs(right.baselineDeltaAmount ?? 0) -
            Math.abs(left.baselineDeltaAmount ?? 0),
        )
        .slice(0, 4),
    [report],
  );
    ...
    >
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <SegmentedControl
            label="Cadence"
            onChange={(value) => updateQuery({ cadence: value, period: null })}
          ...
          {allowHouseholdScope ? (
            <SegmentedControl
              label="Scope"
              onChange={(value) => updateQuery({ scope: value, period: null })}
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border border-[var(--border)] bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-[var(--muted)]">
            The first report will appear after a completed period is processed.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="mt-5 grid gap-5 xl:grid-cols-2">
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 text-xs text-[var(--muted)]">
                            {category.baselineTrend === "NewActivity"
                              ? "New activity"
                              : "Compared with your historical median"}
                          </p>
                        </div>
                        <span
                          className={`inline-flex shrink-0 items-center gap-1 text-sm font-bold ${directionClass}`}
                        >
                          <Icon className="h-4 w-4" />
                          {delta === null
                            ? formatMoney(category.amount, report.currencyCode)
                            : formatSignedMoney(delta, report.currencyCode)}
                        </span>
                      </div>
                    );
                  })
            ...
            <article className="rounded-lg border border-[var(--border)] bg-white p-4 shadow-sm">
              ...
              <div className="mt-4 grid gap-3">
                {report.judgements.length > 0 ? (
                  report.judgements
                    .slice()
                    .sort(
                      (left, right) => right.severityRank - left.severityRank,
                    )
                    .map((finding) => (
                      <FindingCard finding={finding} key={finding.id} />
                    ))
              ...
              {(report.narration?.actions.length ?? 0) > 0 ? (
                <div className="mt-4 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-3">
                  ...
                  <ul className="mt-2 space-y-2 text-sm text-[var(--muted)]">
                    {report.narration!.actions.map((action) => (
...
function SegmentedControl({
  ...
  return (
    <fieldset
      className="flex rounded-lg border border-[var(--border)] bg-white p-1"
      aria-label={label}
    >
      {options.map((option) => (
        <button
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <span
              className={`rounded-md px-2 py-1 text-xs font-bold ${directionClasses}`}
            >
          ...
          <h3 className="mt-2 text-xl font-semibold">
            {report.narration?.headline ?? `${report.cadence} report`}
          </h3>
          <p className="mt-1 text-sm text-[var(--muted)]">
            {formatDate(report.startDate)} –{" "}
            {formatEndDate(report.endDateExclusive)} · {report.timeZone} ·
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="grid h-10 w-10 place-items-center rounded-lg border border-[var(--border)] disabled:opacity-40"
          ...
          <select
            ...
            className="h-10 rounded-lg border border-[var(--border)] bg-white px-3 text-sm font-semibold"
...
function ReportStateBanner({ report }: { report: JudgementReport }) {
  if (
    report.isProcessingUpdate ||
    report.status === "AwaitingNarration" ||
    report.narrationStatus === "Pending"
  ) {
    return (
      <StateBanner
        icon={Clock3}
        text="Updated calculations are ready. Narration is still processing."
        tone="processing"
      />
    );
  }
  if (
    report.narrationStatus === "Fallback" ||
    report.narration?.isDeterministicFallback
  ) {
    return (
      <StateBanner
        icon={CheckCircle2}
        text="Deterministic wording is shown because AI narration was unavailable or not authorized."
        tone="fallback"
      />
    );
  }
  if (report.direction === "InsufficientData") {
    return (
      <StateBanner
        icon={BarChart3}
        text={`Insufficient comparison history. ${report.baselinePeriodsUsed} baseline periods were usable.`}
        tone="fallback"
      />
    );
  }
...
function StateBanner({
  ...
  icon: typeof Clock3;
  text: string;
  tone: "processing" | "fallback";
}) {
  return (
    <p
      className={`mt-4 flex items-center gap-2 rounded-lg border px-4 py-3 text-sm font-semibold ${tone === "processing" ? "border-amber-200 bg-amber-50 text-amber-800" : "border-slate-200 bg-slate-50 text-slate-700"}`}
      role="status"
    >
...
function ReportMetricCard({
  ...
  currencyCode: string;
  metric: JudgementReportMetric;
}) {
  const isRate = rateMetrics.has(metric.code);
  return (
    <article className="rounded-lg border border-[var(--border)] bg-white p-4 shadow-sm">
      <p className="text-xs font-semibold uppercase text-[var(--muted)]">
        {humanize(metric.code)}
      </p>
      <p className="mt-3 text-2xl font-semibold">
        {formatMetric(metric.current, currencyCode, isRate)}
      </p>
      <div className="mt-3 space-y-1 text-xs font-medium text-[var(--muted)]">
        <p>
          Previous: {formatDelta(metric.previousDelta, currencyCode, isRate)}
        </p>
    ...
    <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          ...
          <span
            className={`rounded-md px-2 py-1 text-xs font-bold ${statusClass}`}
          >
      ...
      {finding.value ? (
        <p className="mt-2 text-sm font-bold">{finding.value}</p>
      ) : null}
      {finding.actionCode ? (
        <p className="mt-2 text-xs font-semibold text-[var(--accent)]">
          Action: {humanize(finding.actionCode)}
        </p>
      ) : null}
    </div>
  );
...
function formatDelta(
  ...
  return isRate
    ? `${prefix}${value.toFixed(1)} points`
    : `${prefix}${formatMoney(value, currencyCode)}`;
}
