...
- [API variables](api.env.example)
- [Web variables](web.env.example)
...
Copy the keys from [railway/api.env.example](api.env.example) into the API service Variables page, then replace every placeholder.
...
Copy [railway/web.env.example](web.env.example) into the web service and replace its placeholders:
