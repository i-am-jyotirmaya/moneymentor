...
The pending `apps/landing` PR is separate. Do not statically export `apps/web`; its current standalone Next.js image continues running on EC2. Resend is the email provider. RDS migration, Secrets Manager, CloudFront provisioning, and deployment automation are future work.
...
AWS registration binds configuration without resolving credentials, calling STS, verifying an account, or adding an AWS health check. Startup requires no AWS identity check; an enabled invitation worker may send queued emails as part of its normal work.
...
Future AWS service integrations should reuse this configuration and SDK-managed client lifetime, adding only their required IAM permissions. Email uses Resend independently of AWS. Do not introduce a custom credential cache or AWS types into Application/Domain.
...
All transactional emails use the [Resend HTTPS API](https://resend.com/docs/api-reference/emails/send-email), including MVP approvals, approval resends, and household invitations.
...
After deployment, approve an intended test access request and verify receipt, then enable and test a household invitation. Check application delivery history and the Resend dashboard for failures, bounces, or complaints. A returned message ID means provider acceptance, not guaranteed inbox delivery. Do not log email bodies, tokens, API keys, or recipient details.
...
- For an existing deployment, preserve JWT signing key, issuer, audience, registration mode, and whichever optional workers are currently enabled. Replace SES variables with the Resend settings above; existing queued invitations will send through Resend. Defaults disable optional workers for a fresh smoke test. Changing issuer/audience/key invalidates existing access tokens.
...
- Request-only signup remains enabled. Configure Resend for access-approval emails even if household email dispatch is disabled. See [MVP access operations](../../docs/MVP_ACCESS.md).
...
6. Inspect the instance's attached role/profile and metadata settings in EC2. There is deliberately no startup identity probe: verify email with the approval/invitation send above and inspect the Resend dashboard.
