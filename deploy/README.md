...
AWS credentials come from the EC2 instance role. The application performs no STS identity check; credentials are resolved when sending through SES. An enabled invitation worker processes queued email during normal operation. The pending landing site will later be served from S3 through CloudFront.
