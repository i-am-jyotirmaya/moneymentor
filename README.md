...
- **Resend:** configure `Resend__ApiKey` and `Resend__FromAddress` for all transactional email. `Resend__ReplyTo` is optional. Set `Resend__DispatcherEnabled=true` for household invitations; approval emails send directly regardless of that flag. Verify the sender domain first. Follow [Resend setup](deploy/aws/README.md#resend-email-delivery).
...
- [ ] Configure a Resend API key and verified sender domain; enable the household dispatcher if needed and test delivery.
