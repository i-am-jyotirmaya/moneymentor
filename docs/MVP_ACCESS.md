...
Resend__ApiKey=<Resend sending API key>
Resend__FromAddress=<Resend verified sender>
Resend__ReplyTo=<optional support address>
```
...
The operations executable reads environment configuration; running it from a source checkout only falls back to the development JSON file for the database connection. Configure the Resend API key, verified sender, and public web URL explicitly in its environment. AWS credentials are not required for email. `Resend__DispatcherEnabled` controls household email processing only; it does not disable approval-command emails. Complete [Resend setup](../deploy/aws/README.md#resend-email-delivery) before approving users.
